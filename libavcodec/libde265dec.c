...
static int ff_libde265dec_get_buffer(de265_decoder_context* ctx, struct de265_image_spec* spec, struct de265_image* img, void* userdata)
{
    ...
    const AVPixFmtDescriptor* pfd = av_pix_fmt_desc_get(format);
    if (!pfd || pfd->comp[0].depth != bits_per_pixel) {
        goto fallback;
    }
...
static int ff_libde265dec_decode(AVCodecContext *avctx,
                                 AVFrame *data, int *got_frame, AVPacket *avpkt)
{
    ...
    if ((img = de265_get_next_picture(ctx->decoder)) != NULL) {
        ...
        if (frame != NULL) {
            ...
            if (equal_strides) {
                ...
                const AVPixFmtDescriptor* pfd = av_pix_fmt_desc_get(format);
                int max_bits_per_pixel = pfd->comp[0].depth;
                for (int i=0; i<numplanes; i++) {
...
static av_cold int ff_libde265dec_ctx_init(AVCodecContext *avctx)
{
    ...
    de265_set_parameter_int(ctx->decoder, DE265_DECODER_PARAM_ACCELERATION_CODE, ctx->simd ? de265_acceleration_AUTO : de265_acceleration_SCALAR);
...
#define OFFSET(x) offsetof(DE265Context, x)
#define VD AV_OPT_FLAG_VIDEO_PARAM | AV_OPT_FLAG_DECODING_PARAM
static const AVOption libde265_options[] = {
    { "simd", "SIMD acceleration", OFFSET(simd), AV_OPT_TYPE_BOOL, { .i64 = 1 }, 0, 1, VD },
    { NULL }
};


static const AVClass de265_dec_class = {
    .class_name = "libde265 decoder",
    .item_name  = av_default_item_name,
    .option     = libde265_options,
    .version    = LIBAVUTIL_VERSION_INT,
};

const FFCodec ff_hevc_libde265_decoder = {
    .p.name         = "libde265",
    .p.long_name    = NULL_IF_CONFIG_SMALL("libde265 H.265/HEVC decoder"),
    ...
    .p.capabilities = AV_CODEC_CAP_DELAY | AV_CODEC_CAP_AUTO_THREADS | AV_CODEC_CAP_SLICE_THREADS, //AV_CODEC_CAP_DR1
    .p.priv_class   = &de265_dec_class,
    .caps_internal  = FF_CODEC_CAP_INIT_THREADSAFE |
                      FF_CODEC_CAP_INIT_CLEANUP,
    .p.wrapper_name = "libde265",
};
