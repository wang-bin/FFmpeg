...
static int tls_setup_certificate(TLSContext *s, URLContext *h)
{
    ...
    ret = ptls_minicrypto_load_private_key(&s->ptls_ctx, c->key_file);
    if (ret)
        return log_ptls_error(h, "load private key", ret);
...
static int tls_close(URLContext *h)
{
    ...
    if (s->sign_certificate_inited) {
        free(s->ptls_ctx.sign_certificate);
        s->ptls_ctx.sign_certificate = NULL;
...
static int tls_open(URLContext *h, const char *uri, int flags, AVDictionary **options)
{
    ...
    s->ptls_ctx.random_bytes  = ptls_minicrypto_random_bytes;
    s->ptls_ctx.get_time      = &ptls_get_time;
    s->ptls_ctx.key_exchanges = ptls_minicrypto_key_exchanges;
    s->ptls_ctx.cipher_suites = ptls_minicrypto_cipher_suites;
    ...
    if (!c->listen && c->verify) {
        if (c->ca_file)
            av_log(h, AV_LOG_WARNING, "ca_file is not supported with the picotls/minicrypto backend and will be ignored\n");
        av_log(h, AV_LOG_WARNING, "Certificate chain verification is not available with the picotls/minicrypto backend\n");
    }
